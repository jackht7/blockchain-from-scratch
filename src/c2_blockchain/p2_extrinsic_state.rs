...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for header in chain {
            if hash(current) != header.parent {
                return false;
            }

            if current.height + 1 != header.height {
                return false;
            }

            let expected_state = current.state + header.extrinsic;
            if expected_state != header.state {
                return false;
            }
